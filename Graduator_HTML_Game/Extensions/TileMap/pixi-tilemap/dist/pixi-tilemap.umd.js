...
(function(global, factory) {
  ...
  class RectTileLayer extends display.Container {
    ...
    isModified(anim) {
      if (this.modificationMarker !== this.pointsBuf.length || anim && this.hasAnim) {
        return true;
      }
      return false;
    }
  ...
  class CompositeRectTileLayer extends display.Container {
    ...
    setBitmaps(bitmaps) {
      for (let i2 = 0; i2 < bitmaps.length; i2++) {
        if (bitmaps[i2] && !bitmaps[i2].baseTexture) {
          throw new Error(`pixi-tilemap cannot use destroyed textures. Probably, you passed resources['myAtlas'].texture in pixi > 5.2.1, it does not exist there.`);
        }
      }
  ...
  class MultiTextureResource extends core.resources.Resource {
    ...
    upload(renderer, texture, glTexture) {
      ...
      gl.pixelStorei(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL, texture.alphaMode === void 0 || texture.alphaMode === constants.ALPHA_MODES.UNPACK);
  ...
  function fillSamplers(shader, maxTextures) {
    ...
    let samplerSize = [];
    for (let i = 0; i < maxTextures; i++) {
      samplerSize.push(1 / Constant.bufferSize);
      samplerSize.push(1 / Constant.bufferSize);
    }
...
`;
  ...
  class RectTileGeom extends core.Geometry {
    constructor() {
      ...
      this.addAttribute("aVertexPosition", buf, 0, false, 0, this.stride, 0).addAttribute("aTextureCoord", buf, 0, false, 0, this.stride, 2 * 4).addAttribute("aFrame", buf, 0, false, 0, this.stride, 4 * 4).addAttribute("aAnim", buf, 0, false, 0, this.stride, 8 * 4).addAttribute("aTextureId", buf, 0, false, 0, this.stride, 10 * 4);
    }
  }
  class TileRenderer extends core.ObjectRenderer {
    constructor(renderer) {
      ...
      this.indexBuffer = new core.Buffer(void 0, true, true);
    ...
    bindTexturesWithoutRT(renderer, shader, textures) {
      ...
      for (let i = 0; i < textures.length; i++) {
        ...
        samplerSize[i * 2] = 1 / textures[i].baseTexture.width;
        samplerSize[i * 2 + 1] = 1 / textures[i].baseTexture.height;
      }
      shader.uniforms.uSamplerSize = samplerSize;
    }
    ...
    checkIndexBuffer(size, vb = null) {
      ...
      this.indexBuffer.update(PIXI.utils.createIndicesForQuads(size, Constant.use32bitIndex ? new Uint32Array(size * 6) : void 0));
    }
    getShader() {
  ...
  exports2.CanvasTileRenderer = CanvasTileRenderer;
  exports2.CompositeRectTileLayer = CompositeRectTileLayer;
  exports2.Constant = Constant;
  exports2.GraphicsLayer = GraphicsLayer;
  exports2.MultiTextureResource = MultiTextureResource;
  exports2.POINT_STRUCT_SIZE = POINT_STRUCT_SIZE;
  exports2.RectTileGeom = RectTileGeom;
  exports2.RectTileLayer = RectTileLayer;
  exports2.RectTileShader = RectTileShader;
  exports2.TileRenderer = TileRenderer;
  exports2.TilemapShader = TilemapShader;
  exports2.ZLayer = ZLayer;
  exports2.fillSamplers = fillSamplers;
  exports2.generateFragmentSrc = generateFragmentSrc;
  exports2.generateSampleSrc = generateSampleSrc;
  exports2.pixi_tilemap = pixi_tilemap2;
